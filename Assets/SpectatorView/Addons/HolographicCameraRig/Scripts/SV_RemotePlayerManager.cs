...
namespace SpectatorView
{
    ...
    /// </summary>
    public class SV_RemotePlayerManager : SV_Singleton<SV_RemotePlayerManager>
    {
        ...
        private long prevTimeOffsetNS = 0;
        private float colorFrameReceivedTimeS = 0;
#else
        ...
        private NetworkMovement networkMovement = null;
#endif

        private SV_CustomMessages customMessages = null;
        ...
        void Start()
        {
            customMessages = SV_CustomMessages.Instance;
        ...
        {
            customMessages.MessageHandlers[SpectatorView.SV_CustomMessages.TestMessageID.HeadTransform] = UpdateHeadTransform;
        ...
        void OnUpdateSpectatorViewIP(HoloToolkit.Sharing.NetworkInMessage msg)
        {
            ...
            if (_SpectatorViewIP != SV_Settings.Instance.SV_IP)
            {
                Debug.Log("Updating spectator view ip to: " + SV_Settings.Instance.SV_IP);
                SV_Settings.Instance.SV_IP = _SpectatorViewIP;
                SpectatorView.HolographicCameraManager.Instance.HolographicCameraIP = SV_Settings.Instance.SV_IP;
            }
        }
        ...
        private void SessionUsersTracker_UserJoined(User user)
        {
            ...
            {
                SceneManager.Instance.SendCurrentScene();
            }

            if ((HolographicCameraManager.Instance.LocalComputerIP.Trim() == string.Empty) ||
                HolographicCameraManager.Instance.localIPs.Contains(
                    HolographicCameraManager.Instance.LocalComputerIP.Trim()))
            {
        ...
        public RemoteHeadInfo GetRemoteHeadInfo(long userID)
        {
            ...
            if (headInfo.IP != null 
                ...
                && !HolographicCameraManager.Instance.IsCurrentlyActive 
                && SharingStage.Instance != null 
        ...
        private void SendSpatialMappingData()
        {
            ...
            foreach (MeshFilter filter in meshFilters)
            {
                ...
                SV_CustomMessages.Instance.SendSpatialMapping(vertices, normals, indices);
            }
        }
